...
"#
    ...
    let ok_enum = protocol::v1::ping::Rep::OkEnum(nested_enum.clone());
    let ok_struct = protocol::v1::ping::Rep::OkStruct(nested_struct.clone());

    let ok_enum_dump = ok_enum.clone().dump().unwrap();
    let ok_struct_dump = ok_struct.clone().dump().unwrap();
